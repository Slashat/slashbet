...
app.get('/', function action_index(req, res){
  var result = Bet.all(function(err, result) {
    ...
    res.render('index', { title: 'Slashbet', bets: result.rows, isAuthenticated: (req.isAuthenticated() && (config.auth.github.ids.indexOf(req.user.id) !== -1)) });
  });
});
...
app.get('/:id', function action_single(req, res){
  Bet.getById(req.params.id, function(err, result) {
    ...
    else
    {
      ...
      res.render('single', { title: bet.bet+" <small>("+helper.yyyymmdd(bet.date)+")</small>", bet: bet, percent: percent, color: bet.color });
    }
  });
});
