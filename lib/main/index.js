...
app.get('/', function action_index(req, res){
  var result = Bet.all(function(err, result) {
    ...
    var bets = [];
    var stats = {'active' : 0, 'archived': 0, 'tot_bets': 0, 'yes' : 0, 'no': 0, 'tot_votes': 0};
    result.rows.forEach(function(bet) {
      ...
      stats.active++;
      stats.tot_bets++;
      stats.yes += parseInt(bet.yes, 10);
      stats.no += parseInt(bet.no, 10);
      stats.tot_votes += parseInt(bet.votes, 10);
    });

    var bets_close_enddate = bets.slice(0);
      ...
      res.render('single',
        {
          ...
          percent: percent,
          color: bet.color,
          notArchived: (bet.date.getTime() > (new Date).getTime()),
          date: helper.yyyymmdd(bet.date)
        }
      );
    }
  });
...
app.get('/:id/:vote', function action_vote(req, res){
  ...
  Bet.getById(req.params.id, function(err, result) {
    ...
    {
      var bet = result.rows[0];
      if (bet.date.getTime() < (new Date).getTime())
      {
        res.redirect('/'+bet.id);
      }
      else
