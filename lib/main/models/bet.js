...
var all = function all(callback) {
  db.query('SELECT bets.*, COUNT(CASE WHEN vote IS NOT NULL THEN 1 END) as votes FROM bets LEFT JOIN votes ON bets.id = votes.bet_id WHERE date >= current_timestamp GROUP BY id ORDER BY votes DESC', [], function(err, result) {
    if(err) throw err
    callback(err, result);
  });
}
...
// ### Get all bets ordered by votes
var archived = function all(callback) {
  db.query('SELECT bets.*, COUNT(CASE WHEN vote IS NOT NULL THEN 1 END) as votes FROM bets LEFT JOIN votes ON bets.id = votes.bet_id WHERE date < current_timestamp GROUP BY id ORDER BY votes DESC', [], function(err, result) {
    if(err) throw err
    callback(err, result);
  });
